...
export default async function PayInvitePage({ params }: Props) {
  const user = await requireAuth();
  const { token } = await params;

  let order;
  try {
    order = await courseEnrollmentApplicationService.getByInviteToken(token, user.id);
  } catch (e) {
    if (e instanceof ServiceError && e.code === "FORBIDDEN") {
      return (
        <div className="container mx-auto max-w-lg px-4 py-16 text-center">
          <h1 className="font-display text-xl font-semibold">Erişim reddedildi</h1>
          <p className="mt-2 text-muted-foreground">Bu ödeme linki size ait değil.</p>
        </div>
      );
    }
    notFound();
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-12">
      <h1 className="font-display text-2xl font-semibold">Kişisel ödeme</h1>
      <p className="mt-1 text-muted-foreground">
        Onaylanmış başvurunuz için ödeme adımı · Sipariş #{order.orderNumber.slice(-8)}
      </p>
